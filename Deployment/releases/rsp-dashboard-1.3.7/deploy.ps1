...
.DESCRIPTION
    ...
    What it does:
      ...
      4. Waits up to 60 s for http://localhost:<DASHBOARD_PORT>/health/ready -> 200.
...
if (Test-Path -LiteralPath (Join-Path $scriptRoot 'docker-compose.yml')) {
    Push-Location -LiteralPath $scriptRoot
} else {
    Push-Location -LiteralPath (Split-Path -Parent $scriptRoot)
}
try {
    # --- preflight ----------------------------------------------------------
    if (-not $DryRun) {
    ...
    $dashboardPort = Get-EnvPort 'DASHBOARD_PORT' 3000

    if ($DryRun) {
        Write-Host 'Deployment dry run:'
        Write-Host "    Dashboard URL: http://localhost:$dashboardPort"
        Write-Host "    Readiness URL: http://localhost:$dashboardPort/health/ready"
        return
    }
    ...
    # --- 4. Health check ----------------------------------------------------
    $url = "http://localhost:$dashboardPort/health/ready"
    ...
    for ($i = 0; $i -lt 30; $i++) {
        ...
        if ($code -eq '200') {
            ...
            Write-Host "    Dashboard URL: http://${hostname}:${dashboardPort}"
            Write-Host ''
            Write-Host 'Day-2:'
